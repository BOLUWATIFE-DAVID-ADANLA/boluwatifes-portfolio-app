    ...
    <nav>
      ...
      {isOpen && (
        <div
          ...
          <div
            className={`w-full h-full bg-Background p-5 absolute right-0 top-0 transform transition-transform duration-700 ease-[cubic-bezier(0.4, 0, 0.2, 1)] ${
              isOpen ? "translate-x-0" : "translate-x-full"
            }`}
            onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside
          >
            <button
              className="text-Text text-2xl absolute top-4 right-4"
              onClick={() => setIsOpen(false)}>
              <Closebuttonicon />
            </button>
              ...
              {AppPages.map((link, index) => (
                <li key={index}>
                  <a
                    ...
                    className="block p-4 text-[22px] font-serif font-semibold text-Text hover:bg-blue-500 hover:text-white rounded transition duration-300 ease-in-out"
